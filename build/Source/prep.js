...
const scriptLibraryPathInRepo = configUtils.getConfig(
	"scriptLibraryPathInRepo",
);
const scriptLibraryFolderNameInDefinition = configUtils.getConfig(
	"scriptLibraryFolderNameInDefinition",
);
...
async function prepDockerFile(
	...
	await copyLibraryScriptsForDefinition(
...
async function updateScriptSourcesInDockerfile(
	devContainerDockerfilePath,
	repo,
	release,
	updateScriptSha,
	...
	const devContainerDockerfileModified = await updateScriptSources(
		devContainerDockerfileRaw,
		repo,
		release,
		updateScriptSha,
	...
	await copyLibraryScriptsForDefinition(
		path.dirname(devContainerDockerfilePath),
	);
}
...
async function updateAllScriptSourcesInRepo(repo, release, updateScriptSha) {
	...
	await asyncUtils.forEach(allDefinitions, async (currentDefinition) => {
		...
		const dockerFileBasePath = path.join(
			definitionFolder,
		...
		if (await asyncUtils.exists(dockerFileBasePath)) {
			console.log(
				`(*) Looking for script source in base.Dockerfile for ${currentDefinition.name}.`,
			);
			await updateScriptSourcesInDockerfile(
				dockerFileBasePath,
				repo,
				release,
				updateScriptSha,
		...
		if (await asyncUtils.exists(dockerFilePath)) {
			console.log(
				`(*) Looking for script source in Dockerfile for ${currentDefinition.name}.`,
			);
			await updateScriptSourcesInDockerfile(
				dockerFilePath,
				repo,
...
async function copyLibraryScriptsForDefinition(
	definitionDevContainerJsonFolder,
	isForBuild,
	meta,
) {
	const libraryScriptsFolder = path.join(
		definitionDevContainerJsonFolder,
		scriptLibraryFolderNameInDefinition,
	);
	if (await asyncUtils.exists(libraryScriptsFolder)) {
		await asyncUtils.forEach(
			await asyncUtils.readdir(libraryScriptsFolder),
			async (script) => {
				// Only copy files that end in .sh
				if (path.extname(script) !== ".sh") {
					return;
				}
				const possibleScriptSource = path.join(
					__dirname,
					"..",
					"..",
					scriptLibraryPathInRepo,
				...
				if (await asyncUtils.exists(possibleScriptSource)) {
					const targetScriptPath = path.join(
						libraryScriptsFolder,
						script,
					);
					console.log(
						`(*) Copying ${script} to ${libraryScriptsFolder}...`,
					);
					await asyncUtils.copyFile(
						possibleScriptSource,
						targetScriptPath,
					);
				}
			},
	...
	if (isForBuild && meta) {
		...
		await asyncUtils.writeFile(
			path.join(libraryScriptsFolder, "meta.env"),
			metaEnvTemplate(meta),
		);
	}
}
...
async function copyLibraryScriptsForAllDefinitions() {
	const devcontainerFolders = glob.sync(
		`${path.resolve(__dirname, "..", "..")}/+(containers|container-templates|repository-containers)/**/.devcontainer`,
	);
	await asyncUtils.forEach(devcontainerFolders, async (folder) => {
		console.log(
			`(*) Checking ${path.basename(path.resolve(folder, ".."))} for ${scriptLibraryFolderNameInDefinition} folder...`,
		);
