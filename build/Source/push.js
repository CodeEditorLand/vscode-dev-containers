...
const imageLabelPrefix = configUtils.getConfig(
	"imageLabelPrefix",
	"com.microsoft.vscode.devcontainers",
);
...
async function push(
	...
	const definitionsToPush = definitionId
		? [definitionId]
...
async function pushImage(
	...
	const dockerFileExists = await asyncUtils.exists(
		path.join(dotDevContainerPath, "Dockerfile"),
	);
	const baseDockerFileExists = await asyncUtils.exists(
		path.join(dotDevContainerPath, "base.Dockerfile"),
	);
	const dockerFilePath = path.join(
		dotDevContainerPath,
		`${baseDockerFileExists ? "base." : ""}Dockerfile`,
	);
	...
	for (let i = variants.length - 1; i > -1; i--) {
		...
		if (prepOnly) {
			...
			const versionTags = configUtils.getTagList(
				definitionId,
				release,
				updateLatest,
				registry,
			...
			console.log(
				`(*) Tags:${versionTags.reduce((prev, current) => (prev += `\n     ${current}`), "")}`,
			);
			...
			if (
				...
				const buildParams = versionTags.reduce(
					(prev, current) => prev.concat(["-t", current]),
					[],
				);
				...
				for (let buildArg in buildSettings.buildArgs || {}) {
					buildParams.push(
						"--build-arg",
						`${buildArg}=${buildSettings.buildArgs[buildArg]}`,
					);
				}
				...
				if (buildSettings.variantBuildArgs) {
					for (let buildArg in buildSettings.variantBuildArgs[
						...
						buildParams.push(
							"--build-arg",
							`${buildArg}=${buildSettings.variantBuildArgs[variant][buildArg]}`,
						);
					}
				}
				const spawnOpts = {
				...
				await asyncUtils.spawn(
					...
					[
						...
						dockerFilePath,
						"--label",
						`version=${prepResult.meta.version}`,
						`--label`,
						`${imageLabelPrefix}.id=${prepResult.meta.definitionId}`,
						"--label",
						`${imageLabelPrefix}.variant=${prepResult.meta.variant}`,
						"--label",
						`${imageLabelPrefix}.release=${prepResult.meta.gitRepositoryRelease}`,
						"--label",
						`${imageLabelPrefix}.source=${prepResult.meta.gitRepository}`,
						"--label",
						`${imageLabelPrefix}.timestamp='${prepResult.meta.buildTimestamp}'`,
						"--builder",
						"vscode-dev-containers",
						"--progress",
						"plain",
...
async function flattenBaseImage(
	...
	await asyncUtils.spawn(
		...
		[
			...
			"-d",
			"--name",
			"vscode-dev-containers-build-flatten",
			baseImageTag,
			"bash",
		],
		processOpts,
	...
	const containerInspectOutput = await asyncUtils.spawn(
		"docker",
		["inspect", "vscode-dev-containers-build-flatten"],
	...
	await asyncUtils.exec(
		`docker export vscode-dev-containers-build-flatten | docker import ${importArgs} - ${flattenedBaseImageTag}`,
		processOpts,
	);
	await asyncUtils.spawn(
		"docker",
		["container", "rm", "-f", "vscode-dev-containers-build-flatten"],
		processOpts,
	);
